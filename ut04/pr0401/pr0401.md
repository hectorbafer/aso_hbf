...
Para hacer la práctica, necesitaremos las siguientes máquinas en **VirtualBox** con este hostname:
...
Todas las máquinas tienen solamente un adaptador de red en **puente** para tener acceso a Internet y no requerir de un adaptador secundario. Todo se realizará desde **Powershell**, en el que en la versión **GUI** lo ejecutamos como Administrador y en la versión **CORE** solo basta con poner `powershell`.
...
> 💬 Se nos reiniciarán las máquinas al instante por haberle puesto `-Restart`.
...
Editamos el archivo `hosts` que se encuentra en `C:\Windows\System32\drivers\etc\hosts` para habilitar la resolución local de nombres entre las demás máquinas. Lo editamos con el **bloc de notas** pondremos lo siguiente al final del documento:
```
...
Para editar el archivo `hosts` en **CORE**, pondremos el comando `notepad C:\Windows\System32\drivers\etc\hosts`, se nos abrirá el **Bloc de notas** y lo dejaremos exactamente igual que en la versión **GUI**, es decir, al final del documento pondremos esto:
```
192.168.1.10 HBF-2019
192.168.1.20 HBF-CORE-2019
192.168.1.30 HBF-CORE-2016
...
Para acabar con la configuración inicial, comprobamos que se hagan ping entre ellas, he puesto el comando `ping` + el nombre del **Host** de las máquinas para que aparezcan tanto el nombre y la IP que tienen.
...
### Lista TrustedHosts
En nuestra máquina cliente, que va a ser la versión **GUI**, añadimos los servidores a la lista **TrustedHosts**. Esto permite conectarnos a los servidores. Ponemos los siguientes comandos:
```powershell
...
Ahora haremos lo mismo para los **servidores**, pero antes, tenemos que ejecutar este comando en ambas máquinas para habilitar el acceso remoto:
```powershell
...
Ahora podemos ejecutar comandos de nuestra máquina cliente hacia los servidores. El ejercicio pide que hagamos un usuario que sea `admin-hbf`. Para ello, pondremos el siguiente comando:
```powershell
...
Nos pedirán las credendiales de la máquina servidor, tendremos que poner la **IP** con el **usuario** y la **contraseña**.

![Credenciales2019](Imagenes/credenciales2019.png)

Tendremos que volver a repetir la contraseña y veremos que sí ha creado al usuario.
...
Para conseguir que sea `HTTPS`, es necesario que tengamos un certificado. Al no conseguir uno de terceros, creamos uno que sea autofirmado. Vamos a una máquina servidor, que en mi caso usaré `HBF-CORE-2016` porque le he instalado las **GuestAddition** y es mejor para copiar y pegar. Ponemos el siguiente comando:
```powershell
...
Marcamos la opción de Compartir esta carpeta y pondremos el nombre de la carpeta.
...
Para que sea más seguro, volvemos a Propiedades y vamos a Seguridad. Añadimos al grupo Todos y le damos Control Total
...
> 💬 Me daba error el comando y no he encontrado una solución para ello, pongo una captura del error.
...
Para verlo mejor, con la combinación de teclas de `Win+R` ponemos `certmgr.msc` y veremos el certificado con el nombre del **Host** del servidor.
...
> 💬 Esto tampoco puedo probarlo porque necesitaría el certificado sí o sí.
...
> 💬 También me ha dado error. No puedo terminar de comprobarlo.

## 📌 4. Configuración remota con Windows Admin Center
Descargamos **Windows Admin Center** del enlace https://www.microsoft.com/es-es/windows-server/windows-admin-center. Cuando lo tengamos descargado, procedemos a realizar exactamente los siguientes pasos de la instalación:
...
En el último paso, lo que nos sale es un resumen de lo que va a hacer **Windows Admin Center**, podemos ver que el puerto que va a usar es el `443`.

El siguiente paso es quitar la `Configuración mejorada de Internet Explorer`. Desde el **Administrador del servidor**, clicamos en **Servidor local** y vemos que en **Propiedades**, la configuración de **IE** está activado, esto significa que en cada página que entremos desde Internet Explorer nos la bloquea.
...
Luego, reiniciamos la máquina para que se apliquen los cambios. Si volvemos, veremos que pone **Desactivado**, ahora ya no nos bloqueará ninguna página.

![IE desactivado](Imagenes/IEdesactivado.png)

Cuando hayamos reiniciado la máquina, accedemos al enlace de `https://hbf-2019/` y veremos el mensaje de que esa página no es segura. Clicamos en `Más información` y luego a `Continuar en la página web (no recomendado)`.

![Sitio no seguro](Imagenes/noseguro.png)

Ahora nos aparecerá la siguiente página:

![WAC](Imagenes/PaginaWAC.png)

Ya tenemos instalado Windows Admin Center, lo malo es que ya no tiene soporte para Internet Explorer, con lo que no nos dejará iniciar sesión. Hay dos opciones por las que se puede resolver esto:
1. Descargar otro navegador.
2. Acceder por remoto desde otra máquina.

Para esta práctica, hay que hacer el paso 2.

Desde un Windows 10, añadimos en el fichero hosts los servidores, es decir, pondremos esto al final:
```
192.168.1.20 HBF-CORE-2019
192.168.1.30 HBF-CORE-2016
```

Guardamos el archivo. Ahora tendríamos que acceder desde el navegador poniendo `https://"ip-servidor"`.

> 💬 Como no he conseguido hacer los certificados en el ejercicio anterior, no puedo acceder. Pero con esto ya podríamos conectarnos por remoto hacia **Windows Admin Center**.
