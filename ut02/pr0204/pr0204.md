...
### Apartado 3
Ponemos `fg`, que significa *foreground*. Este comando mueve todos aquellos procesos que están en segundo plano a primer plano.
...
El comando para mover los procesos de segundo a primer plano es `fg`, entonces, para mover un proceso concreto a primer plano, tenemos que poner el comando `jobs` para saber cuántos procesos tenemos parados.

Podemos ver el identificador del proceso en primer lugar y tenerlo más fácil a la hora de manipularse. Ahora, ponemos el comando `jobs` más el modificador `%N`, donde **N** es el número del proceso. Un ejemplo puede ser este:
...
### Apartado 3
Al poner `pstree`, podemos ver en forma de árbol y estructurado con ramificaciones de que los procesos dependen de otros procesos.
...
Para comprobar que se haya eliminado, volvemos a poner `ps aux | egrep sleep` y aparecerá el proceso como **Killed**.
...
Se debería usar kill -9 en lugar de kill sin opciones cuando un proceso se ha quedado atascado, no responde y no puede terminarse de forma normal. A diferencia del comando kill por defecto, que envía una señal de terminación cortés (SIGTERM), el comando kill -9 envía una señal de terminación inmediata (SIGKILL).

#### ¿Qué riesgos implica usar SIGKILL para terminar un proceso?
Utilizar la señal SIGKILL (o kill -9) para terminar un proceso puede ser eficaz, pero conlleva riesgos significativos, ya que fuerza la terminación inmediata sin dar tiempo al proceso para que realice una salida limpia.
