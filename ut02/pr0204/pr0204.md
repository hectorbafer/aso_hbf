...
El comando para mover los procesos de segundo a primer plano es `fg`, entonces, para mover un proceso concreto a primer plano, tenemos que poner el comando `jobs` para saber cuántos procesos tenemos parados. Podemos ver el identificador del proceso en primer lugar y tenerlo más fácil a la hora de manipularse. Ahora, ponemos el comando `jobs` más el modificador `%N`, donde **N** es el número del proceso. Un ejemplo puede ser este:
...
### Apartado 2
Al poner `htop`, vemos una tabla en la que podemos interactuar con ella. Al hacer clic en una columna (fila verde de arriba), se va a filtrar por ello, aparece el bloque en color azul y a la derecha una flecha. Por ejemplo:
