...
Creamos la clave desde nuestra máquina anfitriona con `ssh-keygen`, en mi caso, he puesto la clave en otro directorio para que no se sobreescribiese con la clave de la máquina **ASO Server**.

![Crear clave SSH](Imagenes/CrearClave.png)

Ahora, usamos `scp` y copiamos la clave que acabamos de generar de ésta manera:
```
scp .\.ssh\SERVER-A\id_ed25519.pub hector@192.168.56.10:~
```
...
![Pasar clave de SERVER-A](Imagenes/PasarClave.png)

Para comprobarlo, salimos y volvemos a entrar, y no nos pedirá la contraseña.
