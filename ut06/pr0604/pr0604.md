# 📄 PR0604: Manipulación de colecciones en PowerShell
> 💬 Para hacer los ejercicios, se pondrá a la vez en el mismo cuadro el comando de entrada y de salida acompañado de la captura de pantalla para ver el resultado.

## 📁 Parte 1: Arrays fijos
### 📌 1. Reconfiguración de DNS y Verificación
**Paso 1:** Declara el array con los valores iniciales: `192.168.1.10` (Primario) y `10.0.0.50` (Secundario erróneo).
```powershell
$dns = @("192.168.1.10", "10.0.0.50")
```

> 💬 El primero se encuentra en la posición 0 y el segundo en la posición 1.

**Paso 2:** Muestra en pantalla: “Configuración actual: [Primario] - [Secundario]”.
```powershell
"Configuración actual: $($dns[0]) - $($dns[1])"
```

![Configuración actual](Imagenes/Ej1-2.png)

**Paso 3:** Accede directamente al índice correspondiente y cambia la IP del secundario por la de Google: `8.8.8.8`.
```powershell
$dns[1] = "8.8.8.8"
```

**Paso 4:** Muestra en pantalla el número total de servidores DNS configurados.
```powershell
"Número total de servidores DNS: $($dns.Count)"
```

![Contar DNS](Imagenes/Ej1-4.png)

**Paso 5:** Muestra la configuración final corregida.
```powershell
"Configuración final: $($dns[0]) - $($dns[1])"
```

![Configuración final](Imagenes/Ej1-5.png)

### 📌 2. Rotación de logs de backups (LIFO - Last In, First Out)
**Paso 1:** Declara el array con las siguientes cadenas: `Backup_Lunes.zip`, `Backup_Martes.zip`, `Backup_Miercoles.zip`.
```powershell
$backups = @("Backup_Lunes.zip", "Backup_Martes.zip", "Backup_Miercoles.zip")
```

**Paso 2:** Guarda en una variable `$oldest` el primer elemento.
```powershell
$oldest = $backups[0]
```

**Paso 3:** Guarda en una variable `$newest` el último elemento.
```powershell
$newest = $backups[2]
```

**Paso 4:** El backup del Miércoles ha salido corrupto. Modifica el último elemento del array añadiéndole el texto " (CORRUPTO)" al final del nombre.
```powershell
$backups[2] = $backups[2] + " (CORRUPTO)"
```

**Paso 5:** Imprime un resumen: "Rotación de backups: Del [Viejo] al [Nuevo]".
```powershell
"Rotación de backups: Del $oldest al $($backups[2])"
```

![](Imagenes/Ej2-5.png)

## 📁 Parte 2: ArrayList
### 📌 3. Gestión de Cola de incidencias (Priorización)
**Paso 1:** Crea un `ArrayList` vacío.
```powershell
$incidencias = New-Object System.Collections.ArrayList
```

**Paso 2:** Añade las incidencias: "Monitor parpadea" y "Ratón no va". (Recuerda ocultar la salida por pantalla del `.Add()`).
```powershell
[void]$incidencias.Add("Monitor parpadea")
[void]$incidencias.Add("Ratón no va")
```

**Paso 3:** Llega una urgencia: Inserta en la posición 0 la incidencia: "SERVIDOR CAÍDO".
```powershell
$incidencias.Insert(0, "SERVIDOR CAÍDO")
```

**Paso 4:** El técnico resuelve la incidencia del "Ratón". Búscala por su nombre y elimínala de la lista.
```powershell
$incidencias.Remove("Ratón no va")
```

**Paso 5:** Imprime la lista actual de tareas pendientes y cuenta cuántas quedan.
```powershell
$incidencias
"Total de incidencias: $($incidencias.Count)"
```

![Resultado incidencias](Imagenes/Ej3-5.png)

### 📌 4. Validación de lista negra de IPs (Seguridad)
**Paso 1:** Inicializa un `ArrayList` con las IPs ya bloqueadas: `10.10.10.5`, `192.168.50.4`, `80.80.80.80`.
```powershell
$listaNegra = New-Object System.Collections.ArrayList
[void]$listaNegra.Add("10.10.10.5")
[void]$listaNegra.Add("192.168.50.4")
[void]$listaNegra.Add("80.80.80.80")
```

**Paso 2:** Declara una variable `$nuevaAmenaza = "192.168.50.4"`.
```powershell
$NuevaAmenaza = "192.168.50.4"
```

**Paso 3:** Usa una estructura `if` para:
- Si la IP ya está en la lista, muestra: "La IP [IP] ya estaba bloqueada. No se hace nada."
- Si no está, añádela a la lista y muestra: "IP [IP] añadida a la lista negra."

```powershell
if ($ListaNegra -contains $NuevaAmenaza) {
Write-Host "La IP $NuevaAmenaza ya estaba bloqueada. No se hace nada." -ForegroundColor Yellow
} else {
[void]$ListaNegra.Add($NuevaAmenaza)
Write-Host "La IP $NuevaAmenaza se ha añadido a la lista negra." -ForegroundColor Red
}
```

![Comprobar IP](Imagenes/Ej4-3.png)

**Paso 4:** Muestra la lista final ordenada alfabéticamente.
```powershell
$ListaNegra.Sort()
$ListaNegra
```

![Mostrar lista](Imagenes/Ej4-4.png)

## 📁 Parte 3: Listas genéricas
### 📌 5. Hardening de puertos de Firewall (List[int])
**Paso 1:** Instancia una nueva lista genérica que solo acepte Enteros: `[System.Collections.Generic.List[int]]::new()`.
```powershell
$puertos = [System.Collections.Generic.List[int]]::new()
```

**Paso 2:** Añade los puertos estándar: 80, 443, 53.
```powershell
$puertos.Add(80)
$puertos.Add(443)
$puertos.Add(53)
```

**Paso 3:** Por error, alguien abrió el puerto Telnet (23). Añádelo a la lista.
```powershell
$puertos.Add(23)
```

**Paso 4:** Auditoría de seguridad: Elimina el puerto 23 de la lista.
```powershell
$puertos.Remove(23)
```

![Quitar puerto](Imagenes/Ej5-4.png)

**Paso 5:** Intenta añadir (solo escribe la línea comentada) el texto "HTTP" a la lista y explica en un comentario qué error daría.
```powershell
$puertos.Add("HTTP")
```

![Error](Imagenes/Ej5-5.png)

> 💬 Da error porque la lista solo acepta enteros *(int)* y no texto *(string)*

**Paso 6:** Muestra la lista de puertos permitidos ordenada de menor a mayor.
```powershell
$puertos.Sort()
"Puertos autorizados en el Firewall (Hardened)"
$puertos
```

![Mostrar lista](Imagenes/Ej5-6.png)

### 📌 6. Inventario de servicios críticos (List[string])
**Paso 1:** Crea una lista genérica de Strings a partir de un array existente (`::new($array)`):  
Array origen: `Spooler`, `W3SVC`, `LanmanWorkstation`.
```powershell
$arrayServicios = @("Spooler", "W3SVC", "LanmanWorkstation")
$servicios = [System.Collections.Generic.List[string]]::new([string[]]$arrayServicios)
```

**Paso 2:** Te das cuenta de que falta el servicio de actualizaciones. Añade "wuauserv".
```powershell
$servicios.Add("wuauserv")
```

**Paso 3:** Ordena la lista alfabéticamente.
```powershell
$serviciosOrdenados = $servicios | Sort-Object
```

**Paso 4:** Recorre la lista (foreach) y para cada elemento muestra el mensaje: "Monitorizando servicio: [NombreServicio]... OK".
```powershell
foreach ($servicios in $serviciosOrdenados) {
"Monitorizando servicio: $($servicios) ...OK"}
```

![Monitorizar servicios](Imagenes/Ej6-4.png)

## 📁 Parte 4: Manipulación de texto
### 📌 7. Análisis de Log de usuario
**Entrada:** (Nota los espacios extra al principio y final).
```powershell
$logLine = " User: admin ; IP: 192.168.1.55 ; Status: Failed "
```

**Paso 1:** Limpia los espacios en blanco sobrantes del principio y el final de la cadena original.
```powershell
$logLineLimpio = $logLine.Trim()
```

**Paso 2:** Divide la cadena usando el punto y coma `;` como separador.
```powershell
$partes = $logLineLimpio.Split(";")
```

**Paso 3:** El primer elemento será "User: admin". Vuelve a dividirlo para quedarte solo con "admin".
```powershell
$usuario = $partes[0].Split(":")[1].Trim()
```

**Paso 4:** El segundo elemento es la IP. Límpialo para obtener solo "192.168.1.55".
```powershell
$ip = $partes[1].Split(":")[1].Trim()
```

**Paso 5:** Muestra un mensaje final claro: "ALERTA: El usuario [Usuario] intentó conectar desde [IP]".
```powershell
"ALERTA: El usuario $($usuario) intentó conectar desde $($ip)"
```

![Alerta](Imagenes/Ej7-5.png)

### 📌 8. Generador de CSV para recursos humanos
**Paso 1:** Tienes tres variables con correos: `$m1="ana@empresa.com"`, `$m2="luis@empresa.com"`, `$m3="bea@empresa.com"`.
```powershell
$m1 = "ana@empresa.com"
$m2 = "luis@empresa.com"
$m3 = "bea@empresa.com"
```

**Paso 2:** Crea un array temporal que contenga esas tres variables.
```powershell
$emails = @($m1, $m2, $m3)
```

**Paso 3:** Utiliza el operador `-join` para crear una única cadena de texto separada por comas `,`.
```powershell
$listaEmails = $emails -join ","
```

**Paso 4:** Al resultado final, añádele manualmente el encabezado "EmailAddress," al principio.
```powershell
$csvFinal = "EmailAddress, $($listaEmails)"
```

**Paso 5:** Muestra el resultado, que debería parecerse al contenido de un archivo .csv.
```powershell
$csvFinal
```

![Ver CSV](Imagenes/Ej8-5.png)

---
### [⬅️ Volver a UT06](../index.md)
---