...
> 💬 En mi caso no ha funcionado, pero veríamos lo mismo como hemos hecho con los alumnos en ejercicios anteriores.
...
Creamos el GPO con el nombre de `GPO_HighPerf_Security` y lo vinculamos a nuestro dominio raíz de esta manera:

![GPO dominio raíz](Imagenes/Ej3-1.png)
...
![Habilitar borrar archivo paginación](Imagenes/Ej3-1_1.png)

- Crea y vincula un **Filtro WMI** que haga que esta GPO solo se aplique a equipos que tengan **más de 4 GB de memoria RAM**.

Lo que haremos primero, es crear el Filtro WMI desde **Administración de directivas de grupo**, en **Filtros WMI** hacemos clic derecho y **Nuevo**.

![Crear filtro](Imagenes/Ej3-1_2.png)

Ponemos el nombre y la descripción que sea clara, también, clicaremos en el botón de **Agregar** y añadimos el factor que le queremos hacer el filtro, escribiremos:
```
...
Volvemos al GPO que hemos creado, le damos un clic para entrar en él y en la parte inferor donde pone **Filtrado WMI**, seleccionaremos el filtro que acabamos de crear.

![Seleccionar filtro](Imagenes/Ej3-1_4.png)

Saldrá un aviso de si queremos aplicarlo y le damos que **Sí**.

### 📌 3.2. Diferenciación de Sistema Operativo (Workstation vs Server)
Creamos otro GPO en nuetro dominio raíz con el nombre de `GPO_Clientes_UAC`.

![Crear GPO en dominio raíz](Imagenes/Ej3-2.png)

- Configurar la directiva de UAC para deshabilitar la detección de instalaciones.

Vamos a **Editar** en nuestro GPO y seguimos los pasos de la imagen:

![Deshabilitar detección instalaciones](Imagenes/Ej3-2_1.png)
