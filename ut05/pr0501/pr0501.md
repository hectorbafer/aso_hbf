...
![Contraseña de usuario](Imagenes/Ej3_1.png)
...
![Añadir grupo](Imagenes/Ej3_9.png)

Cuando lo tengamos añadido, tendremos lo siguiente:

![Grupo añadido](Imagenes/Ej3_10.png)
...
- Configurar las horas para que solo se permita el inicio de sesión de los usuarios creados tipo *alumno* **de Lunes a Viernes, de 8:00 a 15:00**. El resto del tiempo debe estar denegado.
- Verifica que el personal (profesor y PAS) sigue teniendo acceso 24/7.

Para que un usuario tenga un uso limitado durante cierto tiempo. Tenemos que ir a `"Propiedades del usuario" → Cuenta → Horas de inicio de sesión` y ahí pondremos manualmente las horas que queramos que el usuario esté conectado.  
En este caso, será de los usuarios que hemos creado que tengan un horario **de 8:00 a 15:00 de lunes a viernes**.
Podemos escoger a varios usuarios escogiéndolos con **Ctrl+clic** y luego hacer clic derecho y `Propiedades → Cuenta → Activar casilla de Horas de sesión → Horas de sesión...`.

![Horas de inicio múltiples](Imagenes/Ej4.png)

Una vez dentro, pondremos las horas del inicio de sesión. Para hacerlo, tendremos que hacer clic y arrastrar las horas que no queramos y luego clicar en **Inicio de sesión denegado**, también podemos elegir el día completo y clicar en la misma opción.

![Horario](Imagenes/Ej4_1.png)

Para comprobar que el usuario `pas_1` y `prof_info_1` tengan acceso **24/7**, hacemos lo mismo, pero como es solo un usuario, no existe tal casilla para habilitar la opción de **Horas de sesión**. Dentro, no se modifica nada, porque tiene la hora de inicio de sesión durante todos los días a todas horas por defecto.

![Ejemplo](Imagenes/Ej4_2.png)

- pas_1

![pas_1](Imagenes/Ej4_3.png)

- prof_info_1

![prof_info_1](Imagenes/Ej4_4.png)
